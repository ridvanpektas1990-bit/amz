...
import { computeInboundLocalDeduction } from "./local-stock.ts";
...
> {
  ...
  if (error) throw new Error(`Local stock: ${error.message}`);
  ...
  for (const row of (data || []) as LocalStockDbRow[]) {
    map.set(String(row.seller_sku), {
      ...
      transferLeadDays: Math.max(
        0,
        Math.round(Number(row.transfer_lead_days) || 7) || 7,
      ),
      lastInboundSeen:
        row.last_inbound_seen == null
          ? null
